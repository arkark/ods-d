...
unittest {
  ...
  assert(list.removeFront() == 10);
  assert(list.size == 2);
  assert(list.get(0) == 3 && list.get(1) == 5);

  assert(list.removeBack() == 5);
  assert(list.size == 1);
  assert(list.get(0) == 3);
...
unittest {
  ...
  // SkiplistList should be able to execute `insertFront`, `insertBack`, `removeFront` and `removeBack` 10^^5 times within 2000 ms because the time complexity is average O(log n)."
