module odsD.test.dataStructure.trie.YFastTrie;

import odsD.dataStructure.trie.YFastTrie;
import odsD.test.util;

unittest {
  writeln(__FILE__, ": Some operations");

  auto trie = new YFastTrie!long();
  assert(trie.size == 0);
...
unittest {
  ...
  auto trie = new YFastTrie!(Vec, ulong, vecToUlong, ulongToVec, "a < b")();
...
unittest {
  ...
  auto trie = new YFastTrie!size_t();
  bool[size_t] aa;
