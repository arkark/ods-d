module odsD.test.dataStructure.scapegoatTree.ScapegoatTree;
...
unittest {
  ...
  auto tree = new ScapegoatTree!(long, "a < b")();
  assert(tree.size == 0);
...
unittest {
  ...
  auto tree = new ScapegoatTree!(Vec, "a.lengthSq < b.lengthSq")();
...
unittest {
  ...
  auto tree = new ScapegoatTree!long();
  bool[long] aa;
...
unittest {
  ...
  // ScapegoatTree should be able to execute `add`, `find`, 'exists' and `remove` 10^^5 times within 2000 ms because the average time complexity is O(log n)."
